...
extern String WIFI_USERNAME;  // ✅ THÊM DÒNG NÀY
...
void handleConnect() {
    ...
    // ✅ LƯU VÀO JSON FILE (/info.dat)
    DynamicJsonDocument doc(4096);
    
    // Load file cũ để giữ lại MQTT config
    ...
    // Cập nhật WiFi config
    doc["WIFI_SSID"] = WIFI_SSID;
    doc["WIFI_PASS"] = WIFI_PASS;
    doc["WIFI_USERNAME"] = WIFI_USERNAME;  // ✅ LƯU USERNAME
...
void main_server_task(void *pvParameters) {
    ...
    server.on("/scan", HTTP_GET, handleScan);      // ← GỌI handleScan()
    server.on("/connect", HTTP_GET, handleConnect); // ← GỌI handleConnect()
